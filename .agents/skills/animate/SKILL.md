...
name: animate
description: >-
  Build an animation from scratch, choosing the correct curve, duration, properties, and interruption behavior.
  Activate when asked to animate UI elements, add motion, build transitions, or make components feel responsive.
---
...
- **`transform` and `opacity` only.** They skip layout and paint and run on the GPU. `width`/`height`/`margin`/`padding`/`top`/`left` trigger all three. (`clip-path` is the sanctioned fourth — see `references/recipes.md`. `height` is tolerated only for accordions, where there's no transform equivalent.)
