...
description: >-
  Audit all animations in a codebase and generate prioritized, actionable improvement plans.
  Activate when auditing existing project animations, fixing sluggish or jarring transitions, or systematically upgrading UI motion.
---
...
The rule catalog with precise values lives in `references/audit.md`. The plan format lives in `references/plan-template.md`. Load them when you audit and when you write plans.
...
Audit against the eight categories in `references/audit.md`:
...
One plan per selected finding, using `references/plan-template.md`, written into `plans/` as `NNN-short-slug.md` (monotonic numbering; respect existing plans). Stamp each plan with the current commit (`git rev-parse --short HEAD`).
