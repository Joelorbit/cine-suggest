...
name: animate-expo
description: >-
  Build fluid animations for React Native and Expo using Reanimated, gesture handlers, and haptics.
  Activate when building mobile animations, sheet interactions, gestural dismissal, or Expo transitions.
---
...
For ready-to-build implementations — press feedback, drag-to-dismiss sheet, swipe-to-delete, collapsing header, list entrances, keyboard-synced UI, tab indicator, screen transitions — see `references/recipes.md`. Load it whenever the request matches one; start from the recipe rather than from a blank file.
